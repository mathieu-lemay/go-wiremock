...
func (c *Client) FindRequestsByCriteria(r *Request) (*journal.FindRequestsByCriteriaResponse, error) {
	requestBody, err := r.MarshalJSON()
	if err != nil {
		return nil, fmt.Errorf("find request by criteria: build error: %w", err)
	}

	res, err := http.Post(fmt.Sprintf("%s/%s/find", c.url, wiremockAdminRequestsURN), "application/json", bytes.NewBuffer(requestBody))
	if err != nil {
		return nil, fmt.Errorf("find request by criteria: request error: %w", err)
	}
	defer res.Body.Close()

	bodyBytes, err := io.ReadAll(res.Body)
	if err != nil {
		return nil, fmt.Errorf("find request by criteria: read response error: %w", err)
	}

	if res.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("find request by criteria: bad response status: %d, response: %s", res.StatusCode, string(bodyBytes))
	}

	var requests journal.FindRequestsByCriteriaResponse
	err = json.Unmarshal(bodyBytes, &requests)
	if err != nil {
		return nil, fmt.Errorf("find request by criteria: read json error: %w", err)
	}
	return &requests, nil
}
...
func (c *Client) DeleteRequestsByCriteria(r *Request) (*journal.DeleteRequestByCriteriaResponse, error) {
	requestBody, err := r.MarshalJSON()
	if err != nil {
		return nil, fmt.Errorf("delete request by criteria: build error: %w", err)
	}

	res, err := http.Post(fmt.Sprintf("%s/%s/remove", c.url, wiremockAdminRequestsURN), "application/json", bytes.NewBuffer(requestBody))
	if err != nil {
		return nil, fmt.Errorf("delete request by criteria: request error: %w", err)
	}
	defer res.Body.Close()

	bodyBytes, err := io.ReadAll(res.Body)
	if err != nil {
		return nil, fmt.Errorf("delete request by criteria: read response error: %w", err)
	}

	if res.StatusCode != http.StatusOK {
		return nil, fmt.Errorf("delete request by criteria: bad response status: %d, response: %s", res.StatusCode, string(bodyBytes))
	}

	var requests journal.DeleteRequestByCriteriaResponse
	err = json.Unmarshal(bodyBytes, &requests)
	if err != nil {
		return nil, fmt.Errorf("delete request by criteria: error unmarshalling response: %w", err)
	}
	return &requests, nil
}
